<template>
  <div class="app-container">
    ...
    <search-panel HeaderIcon="letter" title="信用证">
      ...
      <el-form label-position="left" :model="queryParams" ref="queryForm" size="small" v-show="showSearch"
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="信用证号码" prop="entryName">
              <el-input v-model="queryParams.entryName" placeholder="信用证号码" clearable
                @keyup.enter.native="handleQuery"></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8">
            <el-form-item label="开证申请人" prop="drawer">
              <el-select filterable v-model="queryParams.drawer" placeholder="请选择开证申请人" clearable>
          ...
          <el-col :span="8">
            <el-form-item label="收益人" prop="payee">
              <el-select filterable v-model="queryParams.payee" placeholder="请选择收益人" clearable>
    ...
    <el-row type="flex" :gutter="10" class="mb8" justify="end">
      ...
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['credit:letter:remove']">删 除</el-button>
      </el-col>
    </el-row>
    <el-table v-loading="loading" :summary-method="(param) => getSummaries2(param, totalKeys,zongji)" show-summary
      ...
      <el-table-column show-overflow-tooltip label="收益人" min-width="130" align="left" prop="payee">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_1757265828501258200" :value="scope.row.payee" />
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip label="金融机构" min-width="160" align="left" prop="financialInstitution">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_acceptor" :value="scope.row.financialInstitution" />
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip label="开证金额(万元)" min-width="160" align="right" prop="invoiceAmount">
        <template slot-scope="scope">
      ...
      <el-table-column show-overflow-tooltip label="到期提醒" min-width="120" align="left" prop="remark">
        <template slot-scope="scope">
      ...
      <div v-if="created_successfully == false">
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="80px" label-position="top">
          <el-row :gutter="20">
            <el-col :span="8">
              <el-form-item label="管理编号" prop="managementId">
                <el-input :readonly="title === '修改信用证'" v-model="form.managementId" placeholder="银行承兑管理编号" />
              </el-form-item>
            </el-col>

            <el-col :span="8">
              <el-form-item label="信用证号码" prop="entryName">
                <el-input :readonly="!isEditable" v-model="form.entryName" placeholder="信用证号码" />
              </el-form-item>
            </el-col>


            <el-col :span="8">
              <el-form-item label="开证金额（万元）" prop="invoiceAmount">
                <el-input-number :disabled="!isEditable" class="w" :controls="false" :precision="2"
                  :readonly="!isEditable" type="number" v-model.trim="form.invoiceAmount" placeholder="开证金额" />
              </el-form-item>
            </el-col>


            <el-col :span="8">
              <el-form-item label="开证日期" prop="draftDate">
                <el-date-picker format='yyyy/MM/dd' :picker-options="pickerOptions1" :disabled="!isEditable" clearable
                  v-model="form.draftDate" type="date" value-format="yyyy-MM-dd" placeholder="请选择开证日期">
                </el-date-picker>
              </el-form-item>
            </el-col>
            ...
            <el-col :span="8">
              <el-form-item label="开证申请人" prop="drawer">
                <el-select filterable :disabled="!isEditable" v-model="form.drawer" placeholder="请选择开证申请人">
                  <el-option v-for="dict in dict.type.sys_1757265915323351000" :key="dict.value" :label="dict.label"
                    :value="dict.label"></el-option>
                </el-select>
              </el-form-item>
            ...
            <el-col :span="8">
              <el-form-item label="收益人" prop="payee">
                <el-select filterable :disabled="!isEditable" v-model="form.payee" placeholder="请选择收益人">
                  <el-option v-for="dict in dict.type.sys_1757265828501258200" :key="dict.value" :label="dict.label"
                    :value="dict.label"></el-option>
                </el-select>
              </el-form-item>
            ...
            <el-col :span="8">
              <el-form-item class="el-tiny-numeric" label="保证金比例" prop="marginLevel">
            ...
            <el-col :span="8">
              <el-form-item class="el-tiny-numeric" label="保证金收益率" prop="marginInterestRate">
                <!-- <el-input :readonly="!isEditable" v-model="marginInterestRate" placeholder="保证金收益率" /> -->
                ...
                <tiny-numeric :disabled="!isEditable" :empty-value="0" :readonly="!isEditable" class="w" show-left
                  ...
                  fractionGroupSeparator: '\xA0', // 小数分组分隔符
                  suffix: '%' // 后置标识
                }" placeholder="保证金收益率"></tiny-numeric>
            ...
            <el-col :span="8">
              <el-form-item label="开证费用金额（万元）" prop="ticketProcessingFee">
                <el-input-number :disabled="!isEditable" class="w" :controls="false" :precision="2"
                  :readonly="!isEditable" v-model.trim="form.ticketProcessingFee" placeholder="开证费用金额" />
              </el-form-item>
            </el-col>

            <el-col :span="8">
              <el-form-item label="到期费用金额（万元）" prop="discountedHandlingFee">
                <el-input-number :disabled="!isEditable" class="w" :controls="false" :precision="2"
                  :readonly="!isEditable" v-model.trim="form.discountedHandlingFee" placeholder="到期费用金额" />
              </el-form-item>
            </el-col>
...
<script>
  ...
  export default {
    name: "Credit",
    dicts: ['sys_1757265828501258200', 'sys_1757265915323351000', 'sys_acceptor', 'sys_1827911313532125200'],
    components: {
      CreateSuccess,
      SearchPanel
    },
    ...
    methods: {
      ...
      calculateValues() {
        this.form.changkouedu = (
          this.form.invoiceAmount * (1 - this.toNumberOrDefault(this.form.marginLevel) / 100)
        ).toFixed(2);
      ...
      getList() {
        ...
        listCredit(searchData).then(response => {
      ...
      handleUpdate(row) {
        ...
        getProjectByManagementId(managementId).then(response => {
          ...
          this.form.discountedHandlingFee = (this.toNumberOrDefault(this.form.discountedHandlingFee) || 0) / 10000;
...
<style lang="scss" scoped>
  ::v-deep .el-tiny-numeric .el-form-item__content {
