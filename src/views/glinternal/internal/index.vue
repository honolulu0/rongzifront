<template>
  <div class="app-container">
    ...
    <el-row type="flex" :gutter="10" class="mb8" justify="end">
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
          v-hasPermi="['glinternal:internal:export']">导 出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['glinternal:internal:add']">新 建</el-button>
      </el-col>
      <!-- <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['glinternal:internal:remove']">删 除</el-button>
      </el-col>
      <!--
    ...
    <el-table :summary-method="(param) => getSummaries2(param, totalKeys, zongji)" show-summary  v-loading="loading" :data="internalList" @selection-change="handleSelectionChange"
      ...
      <el-table-column show-overflow-tooltip label="担保金额（万元）" align="center" prop="guaranteeAmount" width="160">
        <template slot-scope="scope">
          <span>{{ formatNumberAsRMB(scope.row.guaranteeAmount) }}</span>
        </template>
      </el-table-column>
      <el-table-column show-overflow-tooltip label="担保余额（万元）" align="center" prop="guaranteeBalance" width="160">
        <template slot-scope="scope">
      ...
      <el-table-column show-overflow-tooltip fixed="right" label="操作" align="center" class-name="''">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-hasPermi="['glinternal:internal:remove']">删除</el-button> -->
          <el-button size="mini" type="text" @click="handleUpdate(scope.row)" v-hasPermi="['glinternal:internal:edit']">
            查 看</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    dicts: ['sys_1778612529468014600', 'sys_1767156259322069000', 'sys_1767154968256577500', 'sys_1767155091485229000',
      ...
      'sys_1767155302261588000', 'sys_1767155825266131000'
    ],
    components: {
      CreateSuccess,
      SearchPanel
    ...
    methods: {
      ...
      handleUpdate(row) {
        ...
        getInternal(id).then(response => {
          ...
          // this.daterangeStartDate2 = response.data.startDate;
          // this.daterangeStartDate2 = response.data.deadline;
      ...
      handleDelete(row) {
        ...
        // this.$modal.confirm('是否确认删除对内担保台账编号为"' + ids + '"的数据项？').then(function () {
        //   return delInternal(ids);
        // }).then(() => {
        //   this.getList();
      ...
      /** 导出按钮操作 */
      handleExport() {
        this.download('glinternal/internal/export', {
          ...this.queryParams
        }, `internal_${new Date().getTime()}.xlsx`)
      },
      /* 上传完成的回调 */
